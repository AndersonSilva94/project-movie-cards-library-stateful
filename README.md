# Projeto Movie Cards Library Stateful :vhs:

### Décimo primeiro projeto do curso de Desenvolvimento Full Stack Web da Trybe.

## Objetivos a serem alcançados no decorrer da construção do projeto:

Desenvolver uma aplicação que consiste em uma biblioteca de cartões de filmes dinâmica utilizando React. A biblioteca é composta por:
...
- Lendo o estado de um componente e usando para alterar o que for exibido no browser;

- Inicializando um componente, dando a ele um estado pré-definido;

- Atualizando o estado de um componente;

- Capturando eventos utilizando a sintaxe do React;

- Criando formulários utilizando sintaxe JSX com as tags : input, textarea, select, form;

- Transmitindo informações de componentes filhos para componentes pais via callbacks
